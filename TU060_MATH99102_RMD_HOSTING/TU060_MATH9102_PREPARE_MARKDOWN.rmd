...
#
# ********************************************************************
needed_packages <- c("tidyverse", "readxl", "lubridate", "jsonlite", "ggplot2", "gtools", "httr", "readr", "psych", "kableExtra", "cowplot", "summarytools")
...
The purpose of this section is to address all required concepts comprehensively and demonstrate coherent understanding of the statistical analysis preparation phase, as covered during the TU060 MATH9102 Module. The following concepts can be considered to be included in the preparation phase:
...
### Quantitative Data:
For numerical data the measurements we need to describe are the center point of the data and the spread of the data. This tells us the overall shape/distribution of the data for the variable of interest. In the sample dataset there are 3 numerical quantitative variables of interest related to student performance. These are: absences, grades in Mathematics and grades in Portuguese. The table below contains statistics that describe the center point (Mean and Median), the spread (Range, IQR, min and max) for each variable measure and the variability (Standard Deviation, Median absolute Deviation). These statistics inform us about the overall shape and distribution of the variables under consideration. The shape of the distribution is important because it will help us during hypothesis testing to determine what sort of tests we can use; for instance, if we can use parametric tests or if we have to revert to the less statistically powerful non-parametric tests.


The table also includes measures of skew, kurtosis and Interquartile range (IQR). These statistics help us to understand if we have unusual aspects to our data such as outliers or gaps or clusters which may shift the distribution, make it more flat or make it more pointy. The trimmed statistics is the mean of the variable with the bottom and top 1% of the data removed and gives us an indication of the impact of outliers. For example, the IQR for the mG3 variable is 6, indicating that we have outliers which will need to be taken into account in later analysis.


In later sections we will look more closely at the distribution of some of these variables but we can see the grade variables approach a normal distribution with some outliers, gaps and clusters and may be candidates for parametric hypothesis tests. The absence variables are definitely not normally distributed on account of positive skew, and we will need to use the non-parametric tests for evaluating any hypotheses based on these variables.
...
tbl_sperf_numerical_stats %>%
  kbl(caption = "Summary statistics for Numerical Data Types") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))
...
#  psych::describe() %>%
#  rownames_to_column(var = "Attribute") %>% data.table() %>%
#  select(Attribute, min,max,median)

###
#tbl_sperf_categorical_stats %>%
#  kbl(caption = "Summary statistics for Numerical Data Types") %>%
#  kable_styling(bootstrap_options = c("striped", "hover"))
...
* What is the difference in time spent socializing between male and female students?
* How much alcohol is consumed at the weekend, and does this differ between male and female students?

### Relational questions:
Relationship questions ask what type of covariation occurs between variables of interest, is their a causal relationship, and how strong is that relationship if one exists.
